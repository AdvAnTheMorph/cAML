...
class GangEffectTest(unittest.TestCase):
    ...
    def test_constructor(self):
        ...
        for i in range(6):
            sub.add(dataset[i])
        effect = GangEffect(sub, {dataset[0]: 1, dataset[1]: 2, dataset[2]: 3,
                                  dataset[3]: 4, dataset[4]: 5, dataset[5]: 6})

        self.assertEqual(sub, effect.subcontext)  # effect.get_subcontext()
        self.assertEqual({"e": {dataset[1], dataset[3], dataset[5]},
                          "r": {dataset[0], dataset[2], dataset[4]}},
                         effect.class_to_instances)
