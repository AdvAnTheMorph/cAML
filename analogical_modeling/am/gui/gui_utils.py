...
class ClsFrame(VisOnCommandFrame):
    def __init__(self, parent: tk.Frame, wrapper):
        ...
        tk.Label(self.frame,
                 justify=tk.LEFT,
                 anchor=tk.W,
                 text=f"{'Class column:':{LEN*2}s}\t").pack(side=tk.LEFT,
                                                          expand=True,
                                                          fill=tk.X)
...
class OutFrame:
    def __init__(self, parent: tk.Frame, wrapper):
        ...
        prefix_frame.pack(side=tk.BOTTOM, after=dir_frame, expand=True, fill=tk.X)
        ...
        self.label = tk.Label(self.frame,
                              ...
                              fg="red")
        self.label.pack(side=tk.BOTTOM, before=prefix_frame, expand=True, fill=tk.X)
...
class MainConfigFrame:
    def __init__(self, app, parent, wrapper):
        ...
        tk.Label(parent, text="Configuration", font=("", 15)).pack(
            expand=True,
            fill=tk.BOTH)
        ...
        tk.Label(lex_frame, text=f"{'Lexicon file:':{LEN}s}\t", justify=tk.LEFT,
                 anchor=tk.W).pack(side=tk.LEFT, expand=True, fill=tk.X)
        self.lexicon_button = tk.Button(lex_frame, text="Select file",
                                        command=self.get_lexicon, width=LEN)
        ...
        tk.Label(test_frame,
                 justify=tk.LEFT,
                 anchor=tk.W,
                 text=f"{'Test file (optional):':{LEN}s}").pack(side=tk.LEFT,
                                                                expand=True,
                                                                fill=tk.X)
        self.test_button = tk.Button(test_frame, text="Select file", width=LEN,
                                     command=self.get_testset)
        self.test_button.pack(side=tk.RIGHT, expand=True)
        self.del_test_button = tk.Button(test_frame, text="Clear file",
                                         command=self.clear_test, width=10)
    ...
    def get_testset(self):
        self.wrapper.testset = self.get_file(self.test_button) or ""
        if self.wrapper.testset:
            self.del_test_button.pack(side=tk.RIGHT, after=self.test_button, expand=True, fill=tk.X)
...
class CountFrame:
    ...
    def __init__(self, parent, wrapper):
        ...
        tk.Label(self.frame,
                 text=f"{'Count strategy:':{LEN}s}\t", justify=tk.LEFT,
