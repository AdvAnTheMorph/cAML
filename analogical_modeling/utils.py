...
class Instance(pd.Series):
    _metadata = ["class_index", "ignored"]

    # FIXME: https://pandas.pydata.org/docs/development/extending.html
    def __init__(self, data: pd.Series, class_index: int, ignore_list: list[str]):
        super().__init__(data)
        self.class_index = class_index
        self.ignored = ignore_list

    def is_missing(self, idx: int) -> bool:
        # return self.isna().iloc[idx]
        return self.index[idx] in self.ignored or self.iloc[idx] == "="
...
class Dataset:
    ...
    def __getitem__(self, idx) -> Instance:
        return Instance(self.data.iloc[idx], self.class_index, self.ignored)

    def __iter__(self):
        for _, el in self.data.iterrows():
            yield Instance(el, self.class_index, self.ignored)
